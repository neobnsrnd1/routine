...
export function HabitEditForm({ habit }: { habit: EditableHabit }) {
  ...
  return <form key={state.resetKey ?? habit.updated_at} action={action} className="mt-3 space-y-2 rounded-md border p-3">
    <input type="hidden" name="habit_id" value={habit.id} />
    <input name="name" defaultValue={habit.name} maxLength={100} required className="h-9 w-full rounded-md border px-3 text-sm" />
    <select name="schedule_type" value={schedule} onChange={(event) => setSchedule(event.target.value as EditableHabit["schedule_type"])} disabled={locked} className="h-9 w-full rounded-md border px-3 text-sm"><option value="daily">매일</option><option value="specific_days">특정 요일</option><option value="weekly_target">주 N회</option></select>
    ...
    {!locked && <input name="start_date" type="date" defaultValue={habit.start_date} required className="h-9 w-full rounded-md border px-3 text-sm" />}
