...
export function calculateDailyStreak(habit: Habit, completions: Completion[], today: string) {
  const done = new Set(
    completions.filter((row) => row.habit_id === habit.id).map((row) => row.completed_date),
  );
  let cursor = done.has(today) ? today : addDays(today, -1);
  let count = 0;
  while (done.has(cursor) && cursor >= habit.start_date) {
    count++;
...
export function calculateSpecificDaysStreak(
  habit: Habit,
...
export function calculateWeeklyTargetStreak(
  habit: Habit,
  completions: Completion[],
  today: string,
) {
  const done = new Set(
    completions
      .filter((row) => row.habit_id === habit.id && row.completed_date >= habit.start_date)
      .map((row) => row.completed_date),
  );
  const target = habit.target_per_week ?? 0;
  let week = monday(today);
  let count = 0;
  const currentWeekCount = Array.from({ length: 7 }, (_, index) =>
    done.has(addDays(week, index)),
  ).filter(Boolean).length;
  if (currentWeekCount < target) week = addDays(week, -7);
  while (addDays(week, 6) >= habit.start_date) {
    const total = Array.from({ length: 7 }, (_, index) => done.has(addDays(week, index))).filter(
      Boolean,
    ).length;
...
export function calculateHabitStreak(habit: Habit, completions: Completion[], today: string) {
  ...
  if (habit.schedule_type === "specific_days")
    return calculateSpecificDaysStreak(habit, completions, today);
