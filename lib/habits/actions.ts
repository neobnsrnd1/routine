...
function parseSchedule(formData: FormData) {
  ...
  if (scheduleType === "specific_days") {
    ...
    if (!values.length || values.some((value) => !Number.isInteger(value) || value < 0 || value > 6) || new Set(values).size !== values.length) return null;
...
export async function updateHabit(_previousState: UpdateHabitState, formData: FormData): Promise<UpdateHabitState> {
  try {
    ...
    if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(habitId)) return { status: "error", message: "수정할 습관을 찾을 수 없습니다." };
    ...
    if (!schedule || !validDate(startDate)) return { status: "error", message: "입력값을 확인해 주세요." };
    const { data: current, error: currentError } = await supabase.from("habits").select("id, schedule_type, days_of_week, target_per_week, start_date").eq("id", habitId).eq("user_id", userId).maybeSingle();
    if (currentError || !current) return { status: "error", message: "수정할 습관을 찾을 수 없습니다." };
    const { count, error: completionError } = await supabase.from("habit_completions").select("id", { count: "exact", head: true }).eq("habit_id", habitId).eq("user_id", userId);
    if (completionError) return { status: "error", message: "습관을 수정하지 못했습니다." };
    const scheduleChanged = current.schedule_type !== schedule.scheduleType || JSON.stringify(current.days_of_week ?? null) !== JSON.stringify(schedule.daysOfWeek) || current.target_per_week !== schedule.targetPerWeek || current.start_date !== startDate;
    if ((count ?? 0) > 0 && scheduleChanged) return { status: "error", message: "완료 기록이 있는 습관은 반복 설정을 변경할 수 없습니다." };
    const { data: updated, error } = await supabase.from("habits").update({ name, schedule_type: schedule.scheduleType, days_of_week: schedule.daysOfWeek, target_per_week: schedule.targetPerWeek, start_date: startDate }).eq("id", habitId).eq("user_id", userId).select("id").maybeSingle();
    if (error) {
      if (scheduleChanged) {
        const { count: currentCompletionCount } = await supabase.from("habit_completions").select("id", { count: "exact", head: true }).eq("habit_id", habitId).eq("user_id", userId);
        if ((currentCompletionCount ?? 0) > 0) return { status: "error", message: "완료 기록이 있는 습관은 반복 설정을 변경할 수 없습니다." };
      }
    ...
    console.error("Unexpected error while updating habit", error instanceof Error ? error.name : "UnknownError");
    return { status: "error", message: "습관을 수정하지 못했습니다." };
  }
}
