...
export async function getStats(today: string, timezone: string) {
  if (
    !validDate(today) ||
    !timezone.trim() ||
    timezone.length > 100 ||
    zoneDate(timezone) !== today
  )
  ...
  try {
    ...
    if (authError || !userId) return { success: false as const, message };
    ...
    const rows = result.rows,
      ...
      activity = Array.from({ length: 7 }, (_, i) => {
        ...
        return {
          date,
          count: rows.filter((r) => r.completed_date === date).length,
        };
      }),
      cards = habits.map((h) => {
        const rate7 = completionRate(h, rows, recent7, today),
          rate30 = completionRate(h, rows, periodStart, today);
        return {
          id: h.id,
          name: h.name,
          schedule: schedule(h),
          ...
          streakCount: calculateHabitStreak(h, rows, today),
          rate7: rate7.rate,
          rate30: rate30.rate,
          rate30Denominator: rate30.denominator,
          completedToday: todaySet.has(h.id),
        };
      }),
      rates = cards.filter((c) => c.rate30Denominator > 0).map((c) => c.rate30);
    return {
